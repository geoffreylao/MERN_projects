...
function handleActionCompute(state, indices, frame) {
    ...
    const incrementCount = (field, condition) => {
        ...
        ___default['default'].update(state.playerCounts, field, (n) => n + 1);
    };
    ...
    const didDashDance = ___default['default'].isEqual(last3Frames, dashDanceAnimations);
...
function handleActionWavedash(counts, animations) {
    const currentAnimation = ___default['default'].last(animations);
    ...
    const recentAnimations = ___default['default'].keyBy(recentFrames, (animation) => animation);
    if (___default['default'].size(recentAnimations) === 2 && recentAnimations[exports.State.AIR_DODGE]) {
            ...
            if (state) {
                ...
                if (state.event !== null) {
                    this.emit(state.event, {
                        combo: ___default['default'].last(this.combos),
...
class ConversionComputer extends events.EventEmitter {
            ...
            if (state) {
                ...
                if (terminated) {
                    this.emit("CONVERSION", {
                        combo: ___default['default'].last(this.conversions),
                        settings: this.settings,
                    });
                }
            }
    ...
    _populateConversionTypes() {
        ...
        const conversionsToHandle = ___default['default'].filter(this.conversions, (conversion) => {
            return conversion.openingType === "unknown";
        });
        // Group new conversions by startTime and sort
        const sortedConversions = ___default['default'].chain(conversionsToHandle)
            .groupBy("startFrame")
            .orderBy((conversions) => ___default['default'].get(conversions, [0, "startFrame"]))
            .value();
        ...
        sortedConversions.forEach((conversions) => {
            ...
            conversions.forEach((conversion) => {
                ...
                const lastMove = ___default['default'].last(conversion.moves);
...
function generateOverallStats(settings, inputs, stocks, conversions, playableFrameCount) {
    const inputsByPlayer = ___default['default'].keyBy(inputs, "playerIndex");
    const originalConversions = conversions;
    const conversionsByPlayer = ___default['default'].groupBy(conversions, (conv) => { var _a; return (_a = conv.moves[0]) === null || _a === void 0 ? void 0 : _a.playerIndex; });
    const conversionsByPlayerByOpening = ___default['default'].mapValues(conversionsByPlayer, (conversions) => ___default['default'].groupBy(conversions, "openingType"));
    const gameMinutes = playableFrameCount / 3600;
    const overall = settings.players.map((player) => {
        const playerIndex = player.playerIndex;
        const playerInputs = ___default['default'].get(inputsByPlayer, playerIndex) || {};
        const inputCounts = {
            buttons: ___default['default'].get(playerInputs, "buttonInputCount"),
            triggers: ___default['default'].get(playerInputs, "triggerInputCount"),
            cstick: ___default['default'].get(playerInputs, "cstickInputCount"),
            joystick: ___default['default'].get(playerInputs, "joystickInputCount"),
            total: ___default['default'].get(playerInputs, "inputCount"),
        };
...
function getOpeningRatio(conversionsByPlayerByOpening, playerIndex, opponentIndices, type) {
    const openings = ___default['default'].get(conversionsByPlayerByOpening, [playerIndex, type]) || [];
    const opponentOpenings = ___default['default'].flatten(opponentIndices.map((opponentIndex) => ___default['default'].get(conversionsByPlayerByOpening, [opponentIndex, type]) || []));
...
function getBeneficialTradeRatio(conversionsByPlayerByOpening, playerIndex, opponentIndices) {
    const playerTrades = ___default['default'].get(conversionsByPlayerByOpening, [playerIndex, "trade"]) || [];
    const opponentTrades = ___default['default'].flatten(opponentIndices.map((opponentIndex) => ___default['default'].get(conversionsByPlayerByOpening, [opponentIndex, "trade"]) || []));
    const benefitsPlayer = [];
    // Figure out which punishes benefited this player
    const zippedTrades = ___default['default'].zip(playerTrades, opponentTrades);
    zippedTrades.forEach((conversionPair) => {
        const playerConversion = ___default['default'].first(conversionPair);
        const opponentConversion = ___default['default'].last(conversionPair);
...
function isCompletedFrame(players, frame) {
    ...
    for (const player of players) {
        const playerPostFrame = ___default['default'].get(frame, ["players", player, "post"]);
        if (!playerPostFrame) {
            return false;
        }
    }
...
function toHalfwidth(str) {
    ...
    const ret = ___default['default'].map(str, (char) => convertChar(char.charCodeAt(0)));
...
function iterateEvents(slpFile, callback, startPos = null) {
    ...
    const commandPayloadBuffers = ___default['default'].mapValues(slpFile.messageSizes, (size) => new Uint8Array(size + 1));
    ...
    switch (command) {
        case exports.Command.GAME_START:
            const getPlayerObject = (playerIndex) => {
                ...
                const nameTagString = iconv__default['default']
                ...
                const displayNameString = iconv__default['default']
                ...
                const connectCodeString = iconv__default['default']
...
class SlpParser extends events.EventEmitter {
    ...
    getLatestFrame() {
        ...
        return ___default['default'].get(allFrames, indexToUse) || null;
    }
    getSettings() {
    ...
    _handlePostFrameUpdate(payload) {
        ...
        if (payload.frame <= exports.Frames.FIRST) {
            ...
            const playersByIndex = ___default['default'].keyBy(this.settings.players, "playerIndex");
    ...
    _handleFrameUpdate(command, payload) {
        ...
        ___default['default'].set(this.frames, [currentFrameNumber, field, payload.playerIndex, location], payload);
        ___default['default'].set(this.frames, [currentFrameNumber, "frame"], currentFrameNumber);
        ...
        if (settings && (!settings.slpVersion || semver__default['default'].lte(settings.slpVersion, "2.2.0"))) {
            ...
            ___default['default'].set(this.frames, [currentFrameNumber, "isTransferComplete"], false);
        }
    }
    _handleItemUpdate(payload) {
        ...
        ___default['default'].set(this.frames, [currentFrameNumber, "items"], items);
    }
    _handleFrameBookend(payload) {
        const latestFinalizedFrame = payload.latestFinalizedFrame;
        const currentFrameNumber = payload.frame;
        ___default['default'].set(this.frames, [currentFrameNumber, "isTransferComplete"], true);
...
class SlippiGame {
    constructor(input, opts) {
        ...
        if (___default['default'].isString(input)) {
            this.input = {
